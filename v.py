...
def quick_test() -> None:
    ...
    zones = [Zone(zone_id="test_1", name="Zona Test 1", location=GeoPoint(19.4326, -99.1332),
...
class BLEBroadcastService(threading.Thread):
    ...
    def run(self):
        ...
        if self._termux_available:
            ...
            self._log("BLE Broadcast en modo DEMO (consola). Los fragmentos no se emiten por radio real.")
            while self._running:
                frag = self.broadcast.get_next_fragment()
                self._log_demo(frag)
                time.sleep(self.interval)
...
class RadioEnvironmentDataProvider(DataProviderBase):
    ...
    def get_zone_data(self, zone: Any) -> Dict[str, Any]:
        ...
        uber_activity = self._uber_detector.analyze_uber_activity()
        analysis['uber_activity'] = uber_activity
        if analysis['devices_detected'] == 0:
            return {}
        env = analysis['environment']
        uber_idx = uber_activity['uber_activity_index']
        conn_score = env['connectivity_score']
        density = env['device_density']
...
""")
        ...
        print("   ATENCIÓN: el archivo 'gradle/wrapper/gradle-wrapper.jar' no puede generarse automáticamente.")
        print("   Para obtenerlo, ejecuta 'gradle wrapper' en la carpeta del proyecto (si tienes Gradle instalado)")
        print("   o copia el JAR de un proyecto Android existente. Android Studio lo descargará al importar el proyecto.")

    def build_with_gradle(self) -> Optional[Path]:
        gradle_bin = shutil.which("gradle")
        if not gradle_bin:
            print("Gradle no encontrado. Compila manualmente.")
            return None
...
# ================================================================================
# SECCIÓN 22: ENTRY POINT FINAL CON SOPORTE DE PUENTE Y OPCIONES
# ================================================================================
def prepare_bridge():
    ...
    print("📌 Para compilar:")
    print("   1. Asegúrate de tener el archivo 'gradle/wrapper/gradle-wrapper.jar'.")
    print("      - Puedes descargarlo de un proyecto Android existente o generarlo con 'gradle wrapper'.")
    print("   2. Abre la carpeta bridge_app en Android Studio o ejecuta './gradlew assembleDebug'.")
    print("   3. Instala el APK en tu teléfono y ejecuta la app.")
    print("   4. Vuelve a ejecutar este script para usar el puente.\n")
...
def main():
    ...
    if "--broadcast" in args:
        ...
        system.start(use_bluetooth=True, allow_sim=not use_bridge, enable_ble_broadcast=True,
                     prompt_text=MEJOR_OPCION_PROMPT_TEXTO, bridge_app=bridge if use_bridge else None)
        while True:
            time.sleep(1)
