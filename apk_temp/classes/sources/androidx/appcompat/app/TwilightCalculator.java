...
class TwilightCalculator {
    ...
    public void calculateTwilight(long time, double latitude, double longitude) {
        ...
        double solarTransitJ2000 = ((double) (J0 + n)) + arcLongitude + (Math.sin(trueAnomaly2) * 0.0053d) + (Math.sin(2.0d * solarLng) * (-0.0069d));
        ...
        if (cosHourAngle >= 1.0d) {
            ...
            long jRound = Math.round((solarTransitJ2000 - ((double) hourAngle)) * 8.64E7d) + UTC_2000;
            this.sunrise = jRound;
            if (jRound >= time || this.sunset <= time) {
                this.state = 1;
            } else {
                this.state = 0;
            }
