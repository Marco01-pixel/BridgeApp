...
public class Oscillator {
    ...
    public void normalize() {
        ...
        while (true) {
            ...
            if (i4 < fArr4.length) {
                ...
                dArr3[i4] = dArr3[i4 - 1] + (((double) h2) * w2);
                i4++;
            } else {
    ...
    double getP(double time) {
        ...
        double t = time2;
        float[] fArr = this.mPeriod;
        double d = fArr[index2] - fArr[index2 - 1];
        double[] dArr = this.mPosition;
        double m = d / (dArr[index2] - dArr[index2 - 1]);
        double p = this.mArea[index2 - 1] + ((((double) fArr[index2 - 1]) - (dArr[index2 - 1] * m)) * (t - dArr[index2 - 1])) + ((((t * t) - (dArr[index2 - 1] * dArr[index2 - 1])) * m) / 2.0d);
        return p;
    }
    ...
    double getDP(double time) {
        ...
        double[] dArr = this.mPosition;
        double m = d / (dArr[index2] - dArr[index2 - 1]);
        double p = (m * t) + (((double) fArr[index2 - 1]) - (dArr[index2 - 1] * m));
        return p;
    }
