    ...
    private static class Arc {
        ...
        Arc(int mode, double t1, double t2, double x1, double y1, double x2, double y2) {
            ...
            if (this.linear || Math.abs(dx) < EPSILON || Math.abs(dy) < EPSILON) {
                ...
                double dy3 = dy2;
                double dx3 = dx2;
                double dHypot = Math.hypot(dy3, dx3);
            ...
            this.mEllipseA = ((double) (z ? -1 : 1)) * dx;
            this.mEllipseB = ((double) (z ? 1 : -1)) * dy;
        ...
        private void buildTable(double x1, double y1, double x2, double y2) {
            ...
            while (true) {
                ...
                double dist2 = dist;
                double dist3 = i;
                double angle = Math.toRadians((dist3 * 90.0d) / ((double) (r12.length - 1)));
            ...
            while (true) {
                if (i3 < this.mLut.length) {
                    double pos = ((double) i3) / ((double) (r2.length - 1));
                    ...
                    if (index >= 0) {
                        ...
                        double d = dArr2[p1];
                        double ans = (((double) p1) + ((pos - d) / (dArr2[p2] - d))) / ((double) (dArr2.length - 1));
                        this.mLut[i3] = ans;
                    }
                    i3++;
                } else {
