...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1610plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m1521toLongimpl(duration, getUnit());
        if (longDelta != Long.MIN_VALUE && longDelta != Long.MAX_VALUE) {
            long j = this.reading;
            newReading = j + longDelta;
            if ((j ^ longDelta) >= 0 && (j ^ newReading) < 0) {
                m1609overflowLRDsOJo(duration);
            }
        } else {
            double delta = Duration.m1518toDoubleimpl(duration, getUnit());
    ...
    private final void m1609overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m1524toStringimpl(duration)) + '.');
    }
}
