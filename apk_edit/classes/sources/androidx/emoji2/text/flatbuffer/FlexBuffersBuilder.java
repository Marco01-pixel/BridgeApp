...
public class FlexBuffersBuilder {
    ...
    private Value createVector(int key, int start, int length, boolean typed, boolean fixed, Value keys) {
        ...
        for (int i = start; i < this.stack.size(); i++) {
            int elemWidth = this.stack.get(i).elemWidth(this.bb.writePosition(), i + prefixElems);
            bitWidth = Math.max(bitWidth, elemWidth);
            if (typed) {
                if (i == start) {
                    vectorType = this.stack.get(i).type;
                    if (!FlexBuffers.isTypedVectorElementType(vectorType)) {
                        throw new FlexBuffers.FlexBufferException("TypedVector does not support this element type");
                    }
                } else if (vectorType != this.stack.get(i).type) {
                    throw new AssertionError();
                }
            }
        }
        ...
        for (int i2 = start; i2 < this.stack.size(); i2++) {
            writeAny(this.stack.get(i2), byteWidth);
        }
        if (!typed) {
            for (int i3 = start; i3 < this.stack.size(); i3++) {
                this.bb.put(this.stack.get(i3).storedPackedType(bitWidth));
            }
        }
        if (keys != null) {
            typedVector = 9;
        ...
        return new Value(key, typedVector, bitWidth, vloc);
    }
