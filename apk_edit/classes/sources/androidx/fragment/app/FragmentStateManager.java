...
class FragmentStateManager {
    ...
    FragmentStateManager(FragmentLifecycleCallbacksDispatcher dispatcher, FragmentStore fragmentStore, ClassLoader classLoader, FragmentFactory fragmentFactory, FragmentState fs) {
        this.mDispatcher = dispatcher;
        this.mFragmentStore = fragmentStore;
        Fragment fragmentInstantiate = fragmentFactory.instantiate(classLoader, fs.mClassName);
        this.mFragment = fragmentInstantiate;
        if (fs.mArguments != null) {
            fs.mArguments.setClassLoader(classLoader);
        }
        fragmentInstantiate.setArguments(fs.mArguments);
        fragmentInstantiate.mWho = fs.mWho;
        fragmentInstantiate.mFromLayout = fs.mFromLayout;
        fragmentInstantiate.mRestored = true;
        fragmentInstantiate.mFragmentId = fs.mFragmentId;
        fragmentInstantiate.mContainerId = fs.mContainerId;
        fragmentInstantiate.mTag = fs.mTag;
        fragmentInstantiate.mRetainInstance = fs.mRetainInstance;
        fragmentInstantiate.mRemoving = fs.mRemoving;
        fragmentInstantiate.mDetached = fs.mDetached;
        fragmentInstantiate.mHidden = fs.mHidden;
        fragmentInstantiate.mMaxState = Lifecycle.State.values()[fs.mMaxLifecycleState];
        if (fs.mSavedFragmentState != null) {
            fragmentInstantiate.mSavedFragmentState = fs.mSavedFragmentState;
        } else {
